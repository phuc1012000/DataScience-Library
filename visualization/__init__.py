...
# distribution
def distribution(
    df, columns=None, column_fig=2, figsize=(18, 4), color=None, compare=None
):
    ...
    # Draw multiple Distribution figure
        ...
        while num_ax < num_ax_limit:
            if compare and column_fig_index < column_fig - 1:
                amount_val = df.loc[df[column_label] == label[column_fig_index]][
                    columns[num_ax]
                ].values
            else:
            ...
            if color is None:
                ...
                sns.distplot(
                    amount_val, ax=ax[column_fig_index], color=color[column_fig_index]
                )

            if compare and column_fig_index < column_fig - 1:
                ax[column_fig_index].set_title(
                    "Distribution of "
                    + str(columns[num_ax])
                    + " of Class: "
                    + str(label[column_fig_index])
                )
            else:
                ax[column_fig_index].set_title(
                    "Distribution of " + str(columns[num_ax])
                )
        ...
        for k, v in df_col_count.items():
            if k in val_count:
                plot_dict[val_count[k]] = (
                    (val_count[k] / sum(val_count.values())) / df_col_count[k]
                ) * 100 - 100
            else:
                plot_dict[0] = 0
